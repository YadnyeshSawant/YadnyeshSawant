...
ROWS = [
    ("host",),
    ("kv", "Now", "MCA Student @ MIT-WPU, Pune"),
    ("kv", "Also", "Student Placement Rep (SPR)"),
    ("kv", "Edu", "B.Sc. CS, Pune University"),
    ("gap",),
    ("sec", "Languages"),
    ("kv", "Core", "Java, Python, C++, C, SQL, PL/pgSQL"),
    ("kv", "Web", "HTML, CSS"),
    ("gap",),
    ("sec", "Tech Stack"),
    ("kv", "Frameworks", "Spring Boot, Hibernate, Servlets"),
    ("kv", "Databases", "PostgreSQL, MySQL"),
    ("kv", "Tools", "Git, GitHub, IntelliJ, VS Code, Linux"),
    ("kv", "Concepts", "System Arch, DSA, AI, Backend Dev"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">YadnyeshSawant@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">YadnyeshSawant</tspan><tspan fill="{MUTED}">@</tspan>'
